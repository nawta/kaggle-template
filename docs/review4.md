...
`/review4` は、5つのレビュアー（Claude 2モデル + Codex 2モデル + CodeRabbit CLI）を並列実行してコードレビューを行い、結果を統合するスキルです。さらに Deepwiki MCP を任意で参照し、外部 OSS の API 仕様を裏取りします。

複数レビュアーの視点を組み合わせることで、単一モデルでは見落としがちなバグやセキュリティ問題を検出します。
...
2. レビュー用プロンプトを構築
3. 5レビュアー（Claude×2 / Codex×2 / CodeRabbit）をバックグラウンドで並列実行
4. 全レビュアーの完了を待機
5. 各レビュアーの出力を統合して返却
6. （任意）Claude が Deepwiki MCP で外部 OSS の API 仕様を裏取り
7. Claude が5者のレビューを分析し、統合レビューを作成

## 統合レビューの形式

最終出力は以下のセクションで構成されます:

- **Critical** - 複数レビュアーが指摘 or 重大な問題
- **Warning** - 注意すべき点
- **Suggestion** - 改善提案
- **Good** - 良い点
- **レビュアー間の見解の相違** - 矛盾がある場合

各指摘には `[Opus4.7]` `[Opus4.6]` `[Codex5.3]` `[Codex5.4]` `[CodeRabbit]` のタグが付き、Deepwiki で裏取りした内容には `[Deepwiki: <repo>]` の引用タグが付きます。
...
| 設定 | デフォルト | 環境変数 |
|------|-----------|----------|
| タイムアウト | 1800秒（30分） | `CODERABBIT_TIMEOUT` |

- `cr` 未インストール時はスキップ（4レビュアーで継続）
- `cr auth login` 未実行時もスキップ扱い

## 前提条件

- `claude` CLI がインストール済みであること
- `codex` CLI がインストール済みであること
- （任意）`cr`（CodeRabbit CLI）がインストール済みかつ `cr auth login` 済みであること
- （任意）`deepwiki` MCP が `claude mcp add` 済みであること（user スコープ推奨）
